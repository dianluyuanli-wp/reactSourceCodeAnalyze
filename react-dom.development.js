...
//  这是用来创建一个字符串，该字符串理应和服务器端渲染的style属性值一致，
//  它绕过了的告警和安全检查，所以这个值用来比较并不安全，这个只能用在开发环境和ssr
//  的校验中
/**
...

{
  ...
  var isTagValidWithParent = function (tag, parentTag) {
    ...
    switch (tag) {
      ...
      case 'thead':
      case 'tr':
        //  这些标签只在少数情况下有效：某些父组件有特殊子元素解析规则。如果我们在这里
        //  这里并没有相匹配的内容，所以我们只有在不知道其父元素的时候允许，其他的情况下
        //  都是非法的
  ...
  var findInvalidAncestorForTag = function (tag, ancestorInfo) {
    switch (tag) {
      ...
      case 'a':
        //  细则说关于存储的一些细节包含一系列标记，但是这听起来和比较一致
  ...
  validateDOMNesting = function (childTag, childText, ancestorInfo) {
    ...
    if (childText != null) {
      //  有childText但是没有childTag,报错validateDOMNesting：当childText传递的时候,childTag不能为null
    ...
    if (invalidParent) {
      ...
      if (ancestorTag === 'table' && childTag === 'tr') {
        //  如果祖先tag是table,warning: 给你的源码添加一个tbody，能够匹配浏览器产生的dom树
        info += ' Add a <tbody> to your code to match the DOM tree generated by ' + 'the browser.';
      }
      //  正在校验Dom嵌套:tagDisplayName不能作为标签ancestorTag的子元素,三个点点点
      warningWithoutStack$1(false, 'validateDOMNesting(...): %s cannot appear as a child of <%s>.%s%s%s', tagDisplayName, ancestorTag, whitespaceInfo, info, addendum);
    } else {
      //  正在校验Dom嵌套:tagDisplayName不能作为ancestorTag的子孙元素
...
//  渲染器不支持持久化，可以从模块中重新导入任何内容
function shim() {
  //  当前的渲染器不支持持久化，这个错误可能是由于react产生的，请给我们反馈
